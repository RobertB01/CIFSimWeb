...
function drawAutomaton(automaton) {
    const width = 1000, height = 500; // Increased canvas size
    ...
    // Create the simulation for layout using Sugiyama-style layering
    const simulation = d3.forceSimulation(automaton.nodes)
        .force("link", d3.forceLink(automaton.links).id(d => d.id).distance(200)) // Increase link distance
        ...
        .force("x", d3.forceX(d => d.layer * 250).strength(1)) // Increase horizontal layer spacing
        .on("tick", ticked);

    // Draw curved edges
    const link = svg.selectAll(".link")
        .data(automaton.links)
        .enter().append("path")  // Changed from line to path for curves
        .attr("class", "link")
        .attr("fill", "none")
        .attr("stroke", "#000")
    ...
    const linkLabels = svg.selectAll(".link-label")
        ...
        .attr("dy", -10) // Increased label distance from links
    ...
    const node = svg.selectAll(".node")
        ...
        .enter().append("circle")
        .attr("class", "node")
        .attr("r", 20) // Increased node size for visibility
        .style("fill", "white")
        .style("stroke", "black")
    ...
    const label = svg.selectAll(".label")
        ...
        .attr("class", "label")
        .attr("dy", ".35em")
        .attr("x", 25) // Move labels further from nodes
